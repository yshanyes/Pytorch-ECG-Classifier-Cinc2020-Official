...
class SEResNetBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        return out
class SEMKResNetBlock(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, groups, reduction, stride=1, downsample=None):
        super(SEMKResNetBlock, self).__init__()

        self.conv_res = nn.Conv1d(inplanes, planes*3, kernel_size=1, padding=0, stride=stride, bias=False)

        self.conv3x3_1 = nn.Conv1d(inplanes, planes, kernel_size=3, padding=1, stride=stride, bias=False)
        self.bn3x3_1 = nn.BatchNorm1d(planes)
        self.conv3x3_2 = nn.Conv1d(planes, planes, kernel_size=3, padding=1, groups=groups, bias=False)
        self.bn3x3_2 = nn.BatchNorm1d(planes)

        self.conv5x5_1 = nn.Conv1d(inplanes, planes, kernel_size=5, padding=2, stride=stride, bias=False)
        self.bn5x5_1 = nn.BatchNorm1d(planes)
        self.conv5x5_2 = nn.Conv1d(planes, planes, kernel_size=5, padding=2, groups=groups, bias=False)
        self.bn5x5_2 = nn.BatchNorm1d(planes)

        self.conv7x7_1 = nn.Conv1d(inplanes, planes, kernel_size=7, padding=3, stride=stride, bias=False)
        self.bn7x7_1 = nn.BatchNorm1d(planes)
        self.conv7x7_2 = nn.Conv1d(planes, planes, kernel_size=7, padding=3, groups=groups, bias=False)
        self.bn7x7_2 = nn.BatchNorm1d(planes)

        self.relu = nn.ReLU(inplace=True)

        self.se_module = SEModule(planes*3, reduction=reduction)
        self.downsample = downsample
        self.stride = stride
    ...
    def forward(self, x):
        ...
        out3 = self.conv3x3_1(x)
        out3 = self.bn3x3_1(out3)
        out3 = self.relu(out3)
        out3 = self.conv3x3_2(out3)
        out3 = self.bn3x3_2(out3)
        out3 = self.relu(out3)

        out5 = self.conv5x5_1(x)
        out5 = self.bn5x5_1(out5)
        out5 = self.relu(out5)
        out5 = self.conv5x5_2(out5)
        out5 = self.bn5x5_2(out5)
        out5 = self.relu(out5)

        out7 = self.conv7x7_1(x)
        out7 = self.bn7x7_1(out7)
        out7 = self.relu(out7)
        out7 = self.conv7x7_2(out7)
        out7 = self.bn7x7_2(out7)
        out7 = self.relu(out7)
...
class SENet(nn.Module):
    ...
    def __init__(self, block, layers, groups, reduction, drop_rate=0.2,
        ...
        self.num_features = 512 * 3#block.expansion
    ...
    def _make_layer(self, block, planes, blocks, groups, reduction, stride=1,
        ...
        if stride != 1 or self.inplanes != planes * 3:#block.expansion
            downsample = nn.Sequential(
                nn.Conv1d(self.inplanes, planes * 3,
...
def semkresnet18(pretrained=False, num_classes=9, in_chans=12, **kwargs):
    model = SENet(SEMKResNetBlock, [2, 2, 2, 2], groups=1, reduction=16,
...
def semkresnet34(pretrained=False, num_classes=9, in_chans=12, **kwargs):

    model = SENet(SEMKResNetBlock, [3, 4, 6, 3], groups=1, reduction=16,
                  inplanes=64, input_3x3=False,
                  downsample_kernel_size=1, downsample_padding=0,
                  num_classes=num_classes, in_chans=in_chans, **kwargs)
    return model

def semkresnet50(pretrained=False, num_classes=9, in_chans=12, **kwargs):
...
def semkresnet101(pretrained=False, num_classes=9, in_chans=3, **kwargs):
    model = SENet(SEMKResNetBlock, [3, 4, 23, 3], groups=1, reduction=16,
                  inplanes=64, input_3x3=False,
                  downsample_kernel_size=1, downsample_padding=0,
                  num_classes=num_classes, in_chans=in_chans, **kwargs)
    return model

def semkresnet152(pretrained=False, num_classes=9, in_chans=3, **kwargs):
    model = SENet(SEMKResNetBlock, [3, 8, 36, 3], groups=1, reduction=16,
                  inplanes=64, input_3x3=False,
                  downsample_kernel_size=1, downsample_padding=0,
                  num_classes=num_classes, in_chans=in_chans, **kwargs)
    return model
...
def semknet154(pretrained=False, num_classes=1000, in_chans=3, **kwargs):
...
if __name__ == '__main__':
    ...
    m = semkresnet18(global_pool='max')
    print(m)
    print(m(x))